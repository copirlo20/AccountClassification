import pandas as pd
import torch
from model import FRIENDS_PATHS, USEFUL_COLS, USER_PATHS, encode_users
...
class Service:
    ...
    def findFriends(self, user_id: int):
    ...
    def build_graph(self, users_id: list[int]):
        friends = []
        edges = []
        index = 0
        for user_id in users_id:
            ...
            index += 1
        friends = encode_users(pd.DataFrame(friends)).reset_index(drop=True)
